...
try {
    ...
    if (Test-Path $ytDlpPath) {
        ...
        if ($fileSize -lt 10MB) {
            throw "Downloaded file size is suspiciously small ($($fileSize / 1KB) KB). Update failed."
        }

        $newVersion = & $ytDlpPath --version 2>$null
        if ($LASTEXITCODE -ne 0 -or [string]::IsNullOrEmpty($newVersion)) {
            throw "Verification failed: The downloaded file is not a valid executable."
        }
    ...
    if (Test-Path $ytDlpBackup) {
        Write-Host "Restoring backup version..." -ForegroundColor Yellow
